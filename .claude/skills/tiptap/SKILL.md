...
Tiptap is already integrated here via `@mantine/tiptap` — see `CLAUDE.md` for the established
conventions (editor lives in `components/Editor/`, extensions are centralized in
`tiptap/extensions.ts`, content is stored/synced as markdown via `contentType: "markdown"`).
Read `CLAUDE.md` first; only fall back to cloning tiptap/tiptap-docs for things it doesn't cover
(new extensions, advanced features below, upstream API details).

Because `@mantine/tiptap`'s `RichTextEditor` component takes an `editor` instance as a prop, this
project uses the classic `useEditor()` hook, not the React Composable API (`EditorProvider` /
`useCurrentEditor`) — the Composable API's context-based pattern doesn't fit how `RichTextEditor`
consumes the editor instance. Don't migrate the existing editor to the Composable API; only use it,
if ever, for new standalone Tiptap usage that doesn't go through `@mantine/tiptap`.
...
- When server-side rendering (e.g. Next.js), set the `immediatelyRender: false` option when initializing the editor. Otherwise, the editor will crash. Learn more about this in tiptap-docs. (Not applicable here — this app is client-only, no SSR.)

### React

- Outside of the `@mantine/tiptap`-based editor (see above), prefer the React Composable API. See `tiptap-docs/src/content/guides/react-composable-api.mdx`
